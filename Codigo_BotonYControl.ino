...
int i = 0;  //  i usado para el for del stepper
int k = 0;  //  auxiliar del for
...
//Infrarrojo, boton y bocina
const int pinBuzzer = 8; //bocina
IRrecv irrecv(A3); //Asigna el pin A3 al infrarrojo
...
/////////////////////////////////////////////////////////////////*/
void setup() {
  irrecv.enableIRIn(); //Habilita recepcion del infrarrojo
  ...
  if(tcs.begin()){
    ...
    digitalWrite(13, HIGH);    
    while(1){
      delay(100);/*tone(pinBuzzer, 440); // bocina
      delay(1000); //detener tono durante 500ms  
      noTone(pinBuzzer);
      delay(500); //generar tono de 523Hz durante 500ms, y detenerlo durante 500ms.
      tone(pinBuzzer, 523, 300);*/
...
void loop() {
  ...
  while (encendido) {
    ...
    analogWrite(11, 30); //Encender cuando este puesto el motor para destrabar
    delay(500);
    analogWrite(11, 0);
    delay(200);
    analogWrite(11, 30);
    ...
    for (i; i<=(k+512); i+=1){  // Stepper va a la posicion i+512 (== to k+512)
      myStepper.step(-1); //Colocar -1 o +1 para cambiar movimiento Horario o Antihorario
      delay(3);
    }
    working = true;
    ...
    while (working) {      
      ...
      //////////////////////////////////
      /////  Check Boton y Control /////
      //////////////////////////////////    
      if (digitalRead(7)==true){ 
        digitalWrite(12, HIGH);
        digitalWrite(13, HIGH);
        Serial.println("EL BOTON ANDA");
        encendido = working = false;
      }
      if (irrecv.decode(&codigo)){
        if (codigo.value==0xFFC23D) {
          digitalWrite(12, HIGH);
          digitalWrite(13, HIGH);
          Serial.println("EL CONTROL ANDA");
          encendido = working = false;
        }
        irrecv.resume(); // Resetea lector del infrarrojo
      }
  ...
  /////  Reanudar sistema /////
  /////////////////////////////
  while (digitalRead(7)==true){     //While para detener el sistema hasta que se suelte el boton
    delay(200);
  }
  

  while (encendido==false){   //Frena el sistema hasta que se aprete el boton o se aprete play en el control
    if (digitalRead(7)==true) {
        Serial.println("EL BOTON ANDA AFUERA");
        encendido = true;
    }

    if (irrecv.decode(&codigo)){
      if (codigo.value==0xFFC23D){
        Serial.println("EL CONTROL ANDA AFUERA");
        encendido = true;
      }
      irrecv.resume(); //Resetea lector del infrarrojo
    }
